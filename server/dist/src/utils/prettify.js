...
var Prettify = /** @class */ (function () {
    function Prettify() {
    }
    Prettify.prototype.BetweenStation = function (string) {
        try {
            var obj = {};
            var retval = {};
            var arr = [];
    ...
    Prettify.prototype.getDayOnDate = function (DD, MM, YYYY) {
        var date = new Date(parseInt(YYYY), parseInt(MM) - 1, parseInt(DD));
        var day = date.getDay() >= 0 && date.getDay() <= 2
            ? date.getDay() + 4
            : date.getDay() - 3;
        return day;
    };
    Prettify.prototype.GetRoute = function (string) {
        try {
            var data = string.split("~^");
            var arr = [];
            var obj = {};
    ...
    Prettify.prototype.LiveStation = function ($) {
        var arr = [];
        var obj = {};
        var retval = {};
    ...
    Prettify.prototype.PnrStatus = function (string) {
        var retval = {};
        var pattern = /data\s*=\s*({.*?;)/;
        var match = string.match(pattern);
        if (match) {
            var data = JSON.parse(match[0].slice(7, -1));
            retval["success"] = true;
            retval["time_stamp"] = Date.now();
            retval["data"] = data;
        }
    ...
    Prettify.prototype.CheckTrain = function (string) {
        try {
            var obj = {};
            var retval = {};
            var data = string.split("~~~~~~~~");
